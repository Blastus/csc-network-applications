...
def main():
    if 'select' in z_cgi.dictionary:
        try:
            ...
            show_first('\n            <h1><font color="red"><u>ERROR</u></font></h1><br/><div style="text-align: left; font-family: Courier;">' + text + '</div>')
    elif 'month' in z_cgi.dictionary:
        show_print()
    else:
        show_first()
...
def create_print(month, year):
    ...
    h_table.set_table(width='100%', height='100%')
    # region V2
    name = 'C:\\Users\\%s\\Desktop' % getpass.getuser()
    if save:
        ...
        stream.maximize('    ')
        stream.parse(xml.sax.saxutils.XMLGenerator(open(os.path.join(name, z_cgi.dictionary['xml']), 'w')))
    # endregion
    data = THIRD_FORM % (m_a_y, h_table.html())
    # region V2
    if z_cgi.dictionary['action'] == 'Create':
        open(os.path.join(name, z_cgi.dictionary['filename']), 'w').write(data)
    # endregion
    z_cgi.print_html(data)
